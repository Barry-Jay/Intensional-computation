...
(* indices in P are renumbered, with binding from left to right *)   
  ...
  | Op o => star_opt (App (App (App (App (Op Eop) (Op o)) (Ref 0)) 
                                  (App k_op (lift 1 M))) 
                             (swap (Ref 0)))
...
Proof.
  ...
  rewrite ! subst_rec_lift_rec1; try omega. auto. 
  (* 1 *) 
  unfold case; fold case.  unfold case_app; fold case_app. unfold swap. 
...
2: unfold swap; unfold_op; auto. rewrite ! lift_rec_preserves_case. 
(* 1 *) 
rewrite star_opt_occurs_true. 2: simpl; rewrite ! orb_true_r; auto. 2: discriminate. 
rewrite star_opt_eta. 2: simpl; rewrite ! orb_false_r; auto.
2: rewrite occurs0_case. 
...
generalize H4; insert_Ref_out; intro. discriminate. 
discriminate.  
gen2_case H1 H4 f.  gen2_case H1 H4 n. discriminate. 
generalize H4; insert_Ref_out; intro. discriminate. 
inversion H4; subst; auto.  discriminate.
gen2_case H1 H4 M1.  gen2_case H1 H4 n. discriminate. 
generalize H4; insert_Ref_out; intro. discriminate. 
...
gen2_case H1 H4 M1.  gen2_case H1 H4 n. discriminate. 
generalize H4; insert_Ref_out; intro. discriminate. 
 discriminate. inversion H4; subst. 
gen2_case H1 H5 f.  gen2_case H1 H5 n. discriminate. 
generalize H5; insert_Ref_out; intro. discriminate. 
inversion H5; subst; auto.  discriminate.
(* 2 *) 
gen2_case H1 H4 M1.  gen2_case H1 H4 n. discriminate. 
generalize H4; insert_Ref_out; intro. discriminate. 
inversion H4; subst; auto.  discriminate.
...
Proof. 
intros; unfold app_comb. eapply2 pnf_compound.  eapply2 pnf_compound. eapply2 pnf_normal. 
Qed. 
...
| App (App (Op Iop) M1) M2 => false
| App (App (Op Kop) M1) M2 => false
| App (App (Op Eop) M1) M2 => false
...
case o;  intro; inversion H; simpl in *; try discriminate;
try eapply2 IHM2;  try inversion H0; try split; auto; try inversion H6.
case o;  intro; split; nf_out; simpl; auto; try discriminate; 
(try assert (program M2) by eapply2 IHM2); try eapply2 H0.
(* 1 *) 
gen_case IHM1 f. 
(* 3 *) 
  split. unfold program. simpl. intro c; inversion c.  
gen_case H0 (maxvar f0); gen_case H0 (maxvar M2); discriminate. 
intro; discriminate. 
(* 2 *) 
split. unfold program.  
gen_case IHM1 o; inversion H; simpl in *; max_out; try discriminate; 
replace (is_program f0) with true; 
replace (is_program M2) with true; auto; 
try (eapply2 sym_eq; eapply2 IHM2; inversion H0;  split; auto; fail); 
try (eapply2 sym_eq; eapply2 IHM1; inversion H0;  split; auto; fail).
(* 6 *) 
inversion H0; simpl in *.  discriminate. inversion H7. 
inversion H0; simpl in *.  discriminate. inversion H7.
(* 4 *)
inversion H0; simpl in *. discriminate. inversion H7. 
(* 3 *)    
inversion H0; simpl in *.  rewrite (closed_implies_passive f0) in H7.
 rewrite (closed_implies_passive M2) in H7; auto. discriminate.
 rewrite (closed_implies_passive f0) in H7; auto.
inversion H7. 
(* 2 *) 
intro. assert(program  (App (Op o) f0)). 
eapply2 IHM1. gen_case H o; try discriminate; 
assert(is_program f0 = true /\ is_program M2 = true) by eapply2 andb_true_iff; 
tauto.  
assert(program M2). eapply2 IHM2. 
gen_case H o; try discriminate; 
assert(is_program f0 = true /\ is_program M2 = true) by eapply2 andb_true_iff; 
tauto.  
split. eapply2 nf_compound. eapply2 H0. eapply2 H1. 
gen_case H o; try discriminate; auto. 
rewrite maxvar_app. 
replace (maxvar(App (Op o) f0)) with 0. replace (maxvar M2) with 0. 
auto. eapply2 sym_eq; eapply2 H1. eapply2 sym_eq; eapply2 H0. 
(* 1 *) 
split; intro; try discriminate. 
inversion H. inversion H0. 
rewrite closed_implies_passive in H6; auto.  discriminate. 
inversion H6. 
Qed. 
...
unfold lift, lift_rec; fold lift_rec. unfold subst, subst_rec; fold subst_rec. 
eapply2 pnf_compound. eapply2 pnf_compound. eapply2 pnf_normal. 
eapply2 pnf_compound. eapply2 pnf_compound. eapply2 pnf_normal. eapply2 pnf_normal. 
2: eapply2 pnf_normal. 
(* 2 *) 
gen_case H M. gen_case H n; relocate_lt;  insert_Ref_out; eapply2 pnf_normal. 
eapply2 pnf_normal. 
(* 2 *) 
rewrite ! occurs_lift_rec_zero. simpl. 
rewrite ! subst_rec_lift_rec; try omega. rewrite ! lift_rec_null.
eapply2 pnf_compound. eapply2 pnf_normal.  eapply2 pnf_compound. eapply2 pnf_normal.
(* 1 *) 
unfold pattern_size; fold pattern_size. 
unfold case; fold case.
assert(maxvar(App P1 P2) = 0 \/ maxvar (App P1 P2) <> 0) by decide equality. 
inversion H0. rewrite H1. 
(* 2 *) 
rewrite star_opt_occurs_true. 2: simpl; rewrite ! orb_true_r; auto. 
2: unfold swap; discriminate. 
rewrite star_opt_occurs_true.  2: simpl; rewrite ! orb_true_r; auto. 2: discriminate. 
rewrite star_opt_eta. 2: rewrite occurs_app. 2: unfold lift; rewrite occurs_lift_rec_zero; auto. 
eapply2 pnf_compound. eapply2 pnf_compound. eapply2 pnf_normal.  
eapply2 pnf_compound. eapply2 pnf_compound. eapply2 pnf_normal.  
eapply2 pnf_compound. eapply2 pnf_normal.  
unfold lift; rewrite subst_rec_lift_rec; try omega. rewrite lift_rec_null. 
eapply2 pnf_closed. normal. eapply2 H2.  
unfold_op. rewrite star_opt_occurs_false.
unfold lift, subst_rec; fold subst_rec.   
rewrite subst_rec_lift_rec; try omega. rewrite lift_rec_null. 
eapply2 pnf_compound. eapply2 pnf_normal.   
eapply2 pnf_compound. eapply2 pnf_normal. 
2: rewrite occurs_app; unfold lift; rewrite  occurs_lift_rec_zero; auto. 
rewrite ! pattern_size_closed. 
replace (j-(0+0)) with j by omega. auto. 
inversion H2.  simpl in H5; max_out. 
inversion H2.  simpl in H5; max_out. 

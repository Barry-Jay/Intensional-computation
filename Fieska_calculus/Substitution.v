...
(*                     Barry Jay                                      *)
(*                                                                    *)
(* adapted from Substitution.v of Project Coq  to act on FIESKA-terms *)
(**********************************************************************)
...
Fixpoint lift_rec (L : Fieska) : nat -> nat -> Fieska :=
  fun k n => 
  match L with
...
Definition lift (n : nat) (N : Fieska) := lift_rec N 0 n.


(* Lifting lemmas *)

Lemma lift_rec_null_term : 
forall (U : Fieska)(n: nat), lift_rec U n 0 = U.
Proof. 
...
forall (M: Fieska), compound M -> forall (n k : nat), compound(lift_rec M n k).
Proof. 
...
gen_case H M1. gen_case H f. 
gen_case H o. rewrite IHp. 2: omega. 
case (status f0); auto. eapply2 IHM2. omega.
gen_case H f1. gen_case H o. eapply2 IHp; omega. omega.
Qed. 
...
Proof. induction M; split_all. discriminate. max_out. rewrite IHM1; auto;  rewrite IHM2; auto. Qed. 
...
Proof. 
induction M; split_all; inversion H; subst; split_all; gen_case H1 M1; try discriminate.
invsub. inversion H1.  gen_case H2 f; try discriminate. inversion H2; subst; auto. 
invsub. 
inversion H1.  gen_case H2 f; try discriminate. inversion H2; subst; auto.
invsub. 
inversion H1.  gen_case H2 f; try discriminate. inversion H2; subst; auto.
invsub. 
invsub. 
...
Definition insert_Ref (N : Fieska) (i k : nat) :=
...
Fixpoint subst_rec (L : Fieska) : Fieska -> nat -> Fieska :=
  fun (N : Fieska) (k : nat) =>
  match L with
...
Definition subst (M N : Fieska) := subst_rec M N 0.


(* The three cases of substitution of U for (Ref n) *)

Lemma subst_eq :
 forall (M U : Fieska) (n : nat), subst_rec (Ref n) U n = lift n U. 
Proof.
...
Lemma subst_lt :
 forall (M U : Fieska) (n p : nat), p > n -> subst_rec (Ref n) U p = Ref n.
Proof.
simpl in |- *; unfold insert_Ref in |- *.
...
forall (W V : Fieska)(n : nat), subst_rec (lift_rec W n 1) V n = W.
Proof.
...
forall (M M' N : Fieska), red M M' -> red  (subst M N) (subst M' N).

Definition subst_preserves_r (red: termred) := 
forall (M N N' : Fieska), red N N' -> red  (subst M N) (subst M N').

Definition subst_preserves (red: termred) := 
forall (M M' : Fieska), red M M' -> forall N N', red N N' -> 
...
forall (M: Fieska), compound M -> forall N, compound(subst M N).
Proof. intros M c; induction c; unfold subst; split_all. Qed. 
Hint Resolve subst_preserves_compound.

Lemma  subst_rec_preserves_components_l : forall (M : Fieska) n k, compound M -> 
...
forall (M : Fieska),  compound M -> forall n k,   
...
(* 1 *) 
rewrite ! subst_rec_preserves_left_component. 
rewrite ! subst_rec_preserves_right_component. 
eapply2 e_compound_compound_red; eapply2 subst_rec_preserves_compounds. 
...
Proof.
induction M; split_all; subst. discriminate. 

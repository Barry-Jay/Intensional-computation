...
Definition termred := Abs_Term -> Abs_Term -> Prop.

Definition preserve (R : termred) (P : Abs_Term -> Prop) :=
  forall x : Abs_Term, P x -> forall y : Abs_Term, R x y -> P y.


Inductive multi_step : termred -> termred :=
  | zero_red : forall red M, multi_step red M M
  | succ_red : forall (red: Abs_Term-> Abs_Term -> Prop) M N P, 
...
Definition reflective red := forall (M: Abs_Term), red M M.
...
Definition transitive red := forall (M N P: Abs_Term), red M N -> red N P -> red M P. 

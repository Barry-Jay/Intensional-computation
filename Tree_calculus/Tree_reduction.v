...
*) 
Definition node := Op Node.
Definition k_op := App node node. 
Definition i_op := App (App node k_op) k_op.
Ltac unfold_op := unfold i_op, k_op, node. 
...
invsub. exist(App (App N'2 x) (App N'5 x)). 
(* 5 *) 
...
(* 3 *) 
apply succ_red with (App (App N P) (App M P)). eapply2 s_sf_seqred. 
eapply2 preserves_app_sf_seqred.
case o.  
apply succ_red with M. 
eapply2 k_sf_seqred. auto. 
red; apply succ_red with  (App (App N P) Q) ; split_all. 
eapply2 preserves_app_sf_seqred.
Qed. 
